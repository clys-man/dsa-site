...
export default function Concepts() {
    ...
    const hasConcepts = concepts.data.length > 0;

    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title="Concepts" />
            <div className={cn("flex h-full flex-1 flex-col gap-4 rounded-xl p-4", !hasConcepts ? "justify-center" : "")}>
                {hasConcepts ? (
                    ...
                    <EmptyState message="Nada por aqui... ainda!" description="Parece que não há conceitos disponíveis. Que tal criar um?" />
                )}

                {hasConcepts && <Paginator paginator={concepts} />}
            </div>
        </AppLayout>
    );
}
